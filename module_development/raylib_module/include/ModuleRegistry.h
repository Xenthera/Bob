...
public:
    struct ModuleBuilder {
        ...
        void class_(const std::string& name, std::function<Value(std::vector<Value>)> constructor) {
            auto ctor = std::make_shared<BuiltinFunction>(name, [constructor](std::vector<Value> args, int line, int col) -> Value {
                return constructor(args);
            });
            exports[name] = Value(ctor);
        }
    };
    ...
    // Installed module support
    void setModulesDirectory(const std::string& dir) { modulesDirectory = dir; }
    std::string getModulesDirectory() const { return modulesDirectory; }
    bool isInstalledModule(const std::string& name) const;
    std::shared_ptr<ModuleDef> loadInstalledModule(const std::string& name);
