...
struct Value {
    ...
    // Constructor for wrapped C++ objects
    template<typename T>
    Value(std::shared_ptr<T> obj, const std::string& typeName) 
        : type(ValueType::VAL_OBJECT), object_value(std::static_pointer_cast<void>(obj)), object_type(typeName) {}
    
    // Helper method to create wrapped objects
    template<typename T>
    ...
    Value(Value&& other) noexcept 
        : type(other.type), string_value(std::move(other.string_value)), bigint_value(std::move(other.bigint_value)), array_value(std::move(other.array_value)), dict_value(std::move(other.dict_value)),
          object_value(std::move(other.object_value)), object_type(std::move(other.object_type)),
        ...
        if (type != ValueType::VAL_STRING && type != ValueType::VAL_BIGINT && type != ValueType::VAL_ARRAY && type != ValueType::VAL_DICT && 
            type != ValueType::VAL_FUNCTION && type != ValueType::VAL_BUILTIN_FUNCTION && type != ValueType::VAL_THUNK && type != ValueType::VAL_OBJECT) {
    ...
    Value(const Value& other) : type(other.type) {
        if (type == ValueType::VAL_STRING) {
            ...
            object_value = other.object_value; // shared object
    ...
    inline bool isObject() const { return type == ValueType::VAL_OBJECT; }
    ...
    inline std::string getType() const {
        switch (type) {
            ...
            case ValueType::VAL_OBJECT: return "object(" + object_type + ")";
    ...
    inline double asNumber() const { 
        ...
        if (isInteger()) return static_cast<double>(integer);
        return 0.0; 
    }
    ...
    // Unwrap C++ objects
    template<typename T>
    std::shared_ptr<T> unwrapAs(const std::string& expectedType) const {
        if (!isObject() || object_type != expectedType) {
            throw std::runtime_error("Expected object of type " + expectedType + ", got " + object_type);
        }
    ...
    inline bool isTruthy() const {
        switch (type) {
            ...
            case ValueType::VAL_OBJECT: return object_value != nullptr;
